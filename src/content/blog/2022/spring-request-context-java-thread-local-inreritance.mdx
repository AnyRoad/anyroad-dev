...
postSlug: spring-request-context-java-thread-local-inreritance
...
```java title="RequestContextHolder.java"
public abstract class RequestContextHolder  {
  ...
  public static void setRequestAttributes(
                         @Nullable RequestAttributes attributes,
                         boolean inheritable) {
    if (attributes == null) {
...
public abstract class Controller {
  ...
  public ControllerResult oneLevel()
                          throws ExecutionException, InterruptedException {
      RequestAttributes originalRequestAttributes = getRequestAttributes();
      int counterValue = counter.incrementAndGet();

      originalRequestAttributes.setAttribute(ATTRIBUTE_NAME,
                                             counterValue,
                                             SCOPE);
      counterValue = (int) originalRequestAttributes.getAttribute(
  ...
  private ControllerResult buildResponse(
                        CompletableFuture<RequestAttributes> requestFuture,
                        int counterValue,
    ...
    try {
      attribute =
          (int) inheritedRequestAttributes.getAttribute(ATTRIBUTE_NAME, SCOPE);
    } catch (Exception ex) {
    ...
    return ControllerResult.builder()
        ...
        .childThreadHasSameRequestAttributes(
          originalRequestAttributes == inheritedRequestAttributes
        )
        .build();
    }
}
...
```java title="InheritableThreadLocalInheritableTest.java"
...
```java title="InheritableThreadLocal.java"
...
```java title="InheritedThreadLocalTest.java"
@Test
...
```java title="ExecutorTtlWrapper.java"
@Override
...
```java title="TransmittableThreadLocalTest.java"
@Test
...
```java title="TransmittableThreadLocalInheritTest.java"
@SpringBootTest(classes = {
...
public Callable<ControllerResult> oneLevelAsync() {
  ...
  int counterValueFromAttributes =
          (int) originalRequestAttributes.getAttribute(ATTRIBUTE_NAME, SCOPE);
